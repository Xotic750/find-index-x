 ...
 * @module find-index-x
 */
"use strict";var findIdx,toLength=_dereq_("to-length-x"),toObject=_dereq_("to-object-x"),isString=_dereq_("is-string"),assertIsFunction=_dereq_("assert-is-function-x"),splitString=!1===_dereq_("has-boxed-string-x"),pFindIndex=Array.prototype.findIndex;findIdx=pFindIndex&&0===[,1].findIndex(function(item,idx){return 0===idx})?function findIndex(array,callback){var object=toObject(array);assertIsFunction(callback);var args=[callback];return arguments.length>2&&(args[1]=arguments[2]),pFindIndex.apply(object,args)}:function findIndex(array,callback){var object=toObject(array);assertIsFunction(callback);var iterable=splitString&&isString(object)?object.split(""):object,length=toLength(iterable.length);if(length<1)return-1;var thisArg;arguments.length>2&&(thisArg=arguments[2]);for(var index=0;index<length;){if(callback.call(thisArg,iterable[index],index,object))return index;index+=1}return-1},module.exports=findIdx},{"assert-is-function-x":2,"has-boxed-string-x":5,"is-string":16,"to-length-x":28,"to-object-x":29}],2:[function(_dereq_,module,exports){/**
